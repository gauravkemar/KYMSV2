...
import com.kemarport.kyms.databinding.FragmentSupervisorBinding
import com.kemarport.kyms.helper.SessionManager

class SupervisorFragment : Fragment(), View.OnClickListener {
    private var _binding: FragmentSupervisorBinding? = null
    private val TAG = "SupervisorFragment"
...
// onDestroyView.
    ...
    override fun onCreateView(
        ...
        _binding = DataBindingUtil.inflate(inflater, R.layout.fragment_supervisor, container, false)
        ...
        toolbar?.let {
            ...
            titleTextView.text = requireActivity().getString(R.string.app_name)
        }
    ...
    override fun onClick(view: View?) {
        view?.let {
            when (it.id) {
                binding.cvPackingList.id -> {
                    val action =
                        SupervisorFragmentDirections.actionSupervisorFragmentToPackingListFragment()
                    findNavController().navigate(action)
                }
                binding.cvVesselLoading.id -> {
                    val action =
                        SupervisorFragmentDirections.actionSupervisorFragmentToVesselLoadingFragment()
                    findNavController().navigate(action)
                }
                binding.cvInterCarting.id -> {
                    val action =
                        SupervisorFragmentDirections.actionSupervisorFragmentToIntercartingFragment()
                    findNavController().navigate(action)
                }
                binding.cvBTS.id -> {
                    val action =
                        SupervisorFragmentDirections.actionSupervisorFragmentToBtsFragment()
                    findNavController().navigate(action)
                }
                binding.cvBTT.id -> {
                    val action =
                        SupervisorFragmentDirections.actionSupervisorFragmentToBttFragment()
                    findNavController().navigate(action)
                }
            }
        }
