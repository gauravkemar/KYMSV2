...
import com.kemarport.kyms.adapters.DelayedEdiAdapter
...
import com.kemarport.kyms.viewmodel.StockTallyWoAsnVM
import com.kemarport.kyms.viewmodel.StockTallyWoAsnVMProviderFactory
    ...
    private fun showAddRakeRefNoDialog() {
        ...
        btnSubmit.setOnClickListener { view: View? ->
            if (etRakeRefNo.getText().toString().trim { it <= ' ' } != "" || etRakeRefNo.getText()
                ...
                val user = session.getUserDetails()
                val jwtToken = user["jwtToken"]
                val createJobMasterRequest = CreateJobMasterRequest(rakeRefNo)
