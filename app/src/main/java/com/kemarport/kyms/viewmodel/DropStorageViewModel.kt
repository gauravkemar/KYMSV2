    ...
    private suspend fun safeAPICallForLocations(
        ...
        try {
            if (hasInternetConnection()) {
                ...
                locationsMutableLiveData.postValue(Resource.Error(NO_INTERNET))
            }
        } catch (t: Throwable) {
            when (t) {
                is IOException -> locationsMutableLiveData.postValue(Resource.Error(NETWORK_FAILURE))
    ...
    private suspend fun safeAPICallAddProductInStock(
        ...
        try {
            if (hasInternetConnection()) {
                ...
                addProductInStockMutableLiveData.postValue(Resource.Error(NO_INTERNET))
            }
        } catch (t: Throwable) {
            when (t) {
    ...
    private suspend fun safeAPICallBackToStock(
        ...
        try {
            if (hasInternetConnection()) {
                ...
                backToStockMutableLiveData.postValue(Resource.Error(NO_INTERNET))
            }
        } catch (t: Throwable) {
            when (t) {
