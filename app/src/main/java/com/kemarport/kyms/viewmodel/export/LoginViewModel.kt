    ...
    private suspend fun safeAPICall(loginRequest: LoginRequest, baseUrl: String) {
        ...
        try {
            if (hasInternetConnection()) {
                ...
                userLoginMutableLiveData.postValue(Resource.Error(Constants.NO_INTERNET))
            }
        } catch (t: Throwable) {
            when (t) {
                is IOException -> userLoginMutableLiveData.postValue(Resource.Error(Constants.NETWORK_FAILURE))
