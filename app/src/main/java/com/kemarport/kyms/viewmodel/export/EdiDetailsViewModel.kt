...
class EdiDetailsViewModel(
    ...
    private fun handleItemScanningResponse(response: Response<ItemScanningResponse>): Resource<com.kemarport.kyms.models.export.itemscanning.ItemScanningResponse> {
        var errorMessage = ""
        if (response.isSuccessful) {
    ...
    private suspend fun safeAPICallUnloadingCoil(
        ...
        try {
            if (hasInternetConnection()) {
                ...
                unloadingCoilMutableLiveData.postValue(Resource.Error(Constants.NO_INTERNET))
            }
        } catch (t: Throwable) {
            when (t) {
    ...
    private suspend fun safeAPICallGetCurrentScanningList(jobId: Int, baseUrl: String, token: String?) {
        ...
        try {
            if (hasInternetConnection()) {
                ...
                currentScanningMutableLiveData.postValue(Resource.Error(Constants.NO_INTERNET))
            }
        } catch (t: Throwable) {
            when (t) {
