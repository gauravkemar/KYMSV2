...
import com.kemarport.kyms.models.export.DelayedEDI.DelayedEDIResponseItem
import java.text.SimpleDateFormat
import java.util.*

class DelayedEdiAdapter : RecyclerView.Adapter<DelayedEdiAdapter.EdiViewHolder>() {

    inner class EdiViewHolder(val binding: ItemEdiBinding) :
        RecyclerView.ViewHolder(binding.root)

    private val differCallback = object : DiffUtil.ItemCallback<DelayedEDIResponseItem>() {
        override fun areItemsTheSame(oldItem: DelayedEDIResponseItem, newItem: DelayedEDIResponseItem): Boolean {
            return oldItem.jobId == newItem.jobId
        }

        override fun areContentsTheSame(
            oldItem: DelayedEDIResponseItem,
            newItem: DelayedEDIResponseItem
        ): Boolean {
    ...
    override fun onBindViewHolder(holder: EdiViewHolder, position: Int) {
        ...
        holder.binding.apply {
            ...
            when (ediResponseItem.jobStatus) {
                ...
                "Unloading Begin" -> {
                    jobStatus = "Unloading in progress"
                }
                "UnLoading Completed" -> {
                    jobStatus = "Unloading completed"
                }
                "Pending" -> {
                    jobStatus = "Pending"
                }
    ...
    private var onItemClickListener: ((DelayedEDIResponseItem) -> Unit)? = null

    fun setOnItemClickListener(listener: (DelayedEDIResponseItem) -> Unit) {
